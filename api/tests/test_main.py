...
def test_predict_endpoint_success(test_client: TestClient, pipeline_instance, valid_image_bytes: bytes, mocker):
  mocked_success = {
      "guard_status": "PASSED",
  ...
  mocker.patch.object(pipeline_instance, 'predict', return_value=mocked_success)
...
def test_predict_endpoint_pipeline_error(test_client: TestClient, pipeline_instance, valid_image_bytes, mocker):
  ...
  assert response.json()["detail"] == "Specific error"
