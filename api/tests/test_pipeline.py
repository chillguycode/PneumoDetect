...
def test_pipeline_initialization(pipeline_instance: InfPipeline):
  ...
  assert pipeline_instance.guard_session is not None
  assert pipeline_instance.main_session is not None
  assert pipeline_instance.MAIN_CLASS_NAMES == ["NORMAL", "PNEUMONIA"]


# Test 2: Initialisation failure with a bad path
def test_pipeline_init_fails_with_bad_path():
  with pytest.raises(Exception):
      InfPipeline(guard_model_path="bad/path.onnx", main_model_path="bad/path.onnx")
...
def test_predict_success(pipeline_instance: InfPipeline, valid_image_bytes: bytes):
  ...
  assert result["prediction"] in pipeline_instance.MAIN_CLASS_NAMES
...
def test_predict_invalid_bytes(pipeline_instance: InfPipeline, non_image_bytes: bytes):
  ...
  assert result["error"] == "Failed to process image or run inference"
